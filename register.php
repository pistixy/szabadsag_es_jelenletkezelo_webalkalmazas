...
$tax_number = $entity_id = $employee_card_number = $letterCode = '';
...
$employee_card_number = filter_input(INPUT_POST, 'employee_card_number', FILTER_SANITIZE_STRING);
...
if ($stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    $stmt = $conn->prepare("INSERT INTO users (
        name, email, password, cim, tax_number, entity_id, employee_card_number, position,
        faculty, paid_free, paid_requested, paid_planned, paid_taken
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

    $stmt->execute([
        $name, $email, $hashed_password, $cim, $tax_number, $entity_id,
        $employee_card_number, $position, $letterCode, $paid_free, $paid_requested,
